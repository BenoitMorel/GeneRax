...
void initStartingSpeciesTree(SpeciesSplitArguments &args,
    Families &families)
{
...
void run( SpeciesSplitArguments &args)
{
  ...
  SpeciesSplitSpeciesTreeOptimizer speciesTreeOptimizer(
      args.speciesTree,
      families,
      args.output,
      args.rootedScore);
...
int genetegrator_main(int argc, char** argv, void* comm)
{
  ...
  Logger::init();
  Logger::timed << "SpeciesSplit v0.0.0" << std::endl; 
  SpeciesSplitArguments args(argc, argv); 
  run(args);
  Logger::close();
