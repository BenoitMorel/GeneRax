...
ICCalculatorSlow::ICCalculatorSlow(const std::string &referenceTreePath,
...
void ICCalculatorSlow::_computeRefBranchIndices()
{
...
void ICCalculatorSlow::_initScores()
{
...
void ICCalculatorSlow::_readTrees(const Families &families)
{
...
void ICCalculatorSlow::_getSpidUnderNode(pll_unode_t *node, TaxaSet &taxa)
{
  if (node->next) {
...
void ICCalculatorSlow::_computeQuartets(bool paralogy)
{
  ...
  if (paralogy) {
    Logger::info << "Accounting for paralogy!" << std::endl;
  } else {
    Logger::info << "NOT accounting for paralogy!" << std::endl;
  }
...
void ICCalculatorSlow::_computeQuartetsForTree(PLLUnrootedTree &evaluationTree)
{
...
void ICCalculatorSlow::_computeQuartetsForTreePro(PLLUnrootedTree &evaluationTree)
{
...
void ICCalculatorSlow::_updateFromMetaquartet(const MetaQuartet &m)
{
    for (auto a: m[0]) {
      for (auto b: m[1]) {
        if (a == b) {
...
void ICCalculatorSlow::_computeScores()
{
...
void ICCalculatorSlow::_processNodePair(pll_unode_t *u, pll_unode_t *v)
{
  if (u == v) {
    return;
  }
...
void ICCalculatorSlow::printNQuartets(unsigned int n) {
  unsigned int idx = 0;
  for (auto a: _allSPID) {
    for (auto b: _allSPID) {
      if (a == b) {
...
void ICCalculatorSlow::_printQuartet(SPID a, SPID b, SPID c, SPID d)
{
...
double ICCalculatorSlow::_getQic(SPID a, SPID b, SPID c, SPID d)
{
...
std::string ICCalculatorSlow::_getNewickWithScore(std::vector<double> &branchScores, const std::string &scoreName)
{
