...
//  std::string signal_string;
  ...
  //std::shared_ptr<bpp::PhyloNode> S_root;                                        //Root of the species tree
  std::map<std::shared_ptr<bpp::PhyloNode>, int> node_ids;                         //Map between node and its id.
  std::map<int, std::shared_ptr<bpp::PhyloNode>> id_nodes;                         //Dual from the above, map between node id and node.
